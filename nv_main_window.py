...
class ToolBarOfClasses(QToolBar):
    ...
    def act_triggered(self):
        ...
        self.send_class_name.emit({"class_name": cls_name})


class AttribColumn(QWidget):
    new_name_value_ac = pyqtSignal(dict)
    ...
    def edit_finished(self):
        ...
        self.new_name_value_ac.emit({"attrib_name": label.text(), "attrib_value": sender.text()})

    @pyqtSlot(str)
    def changed_value(self, new_val: str):
        sender = self.sender()
        label: QLabel = self.widgets_dict[sender]
        self.new_name_value_ac.emit({"attrib_name": label.text(), "attrib_value": new_val})
...
class ToolBarOfAttributes(QToolBar):
    new_name_value_tb = pyqtSignal(dict)
...
class CustomTW(QTreeView):
    ...
    send_data_edit = pyqtSignal(dict)
    send_data_right_click = pyqtSignal(str)
    send_data_hover = pyqtSignal(dict)
    send_data_pick = pyqtSignal(dict)
    ...
    def release_data_emit(self):
        if not (self.data_release is None):
            self.send_data_pick.emit({"picked_obj_name": self.data_release})
            self.timer_double_click.stop()
    ...
    def timeout_handling(self):
        if self.first_timer_notification:
            ...
            if ohn and (ohn not in self.parent().class_nodes):
                self.send_data_hover.emit({"hovered_obj_name": self.obj_hovered_name})
    ...
    def send_name_edit(self, val):
        if val.data() not in self.parent().class_nodes:
            self.send_data_edit.emit({"edit_obj": val.data()})
